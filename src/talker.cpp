...
int main(int argc, char **argv)
{
    ...
    //    OpenSim::Model* model = new OpenSim::Model(
    //            "/home/parallels/Documents/NRP/GazeboRosPackages/src/exoskeleton/resource/arm26.osim");
    //OpenSim::Umberger2010MuscleMetabolicsProbe* probe = new OpenSim::Umberger2010MuscleMetabolicsProbe();

    //    model->addProbe((OpenSim::Probe*) probe);
    //    probe->addMuscle("TRIlong", 0.5);
    //    probe->useProvidedMass("TRIlong", 1.2);
    //    ((OpenSim::Probe *)probe)->setOperation("value");
    //    OpenSim::Array<std::string>* names = new OpenSim::Array<std::string>();
    //    std::ostringstream msg;
    //    msg << "muscle size: " << model->getActuators().getSize() << std::endl;
    //    ROS_INFO("%s", msg.str().c_str());

    testProbesUsingMillardMuscleSimulation();
...
//        //info_msg << "report_total: " << probe->get_report_total_metabolics_only() << std::endl;
//
//        ROS_INFO("%s", info_msg.str().c_str());
//
//        ros::spinOnce();
//
//        loop_rate.sleep();
